...
const tabs = {
    ru: ['Опыт работы', 'Навыки', 'Образование', 'Обо мне'],
    en: ['Work Experience', 'Skills', 'Education', 'About'],
}
    ...
    <div class="px-4 sm:px-8">
        <nav v-if="screenWidth! < 1024"
            ...
            <button v-for="(tab, index) in tabs[$currentSiteLanguage]"
                :class="activeTab[tab] ? ['transition-colors duration-500 text-blue-600 border-blue-600 border-b-2'] : ['border-b-1  border-slate-500 text-slate-500']"
                :data-tab="index + 1"
        ...
        <div :class="screenWidth! >= 1024 ? 'grid grid-cols-2 gap-16' : ''">
            ...
            <slot v-if="activeTab[tabs[$currentSiteLanguage][1]] || screenWidth >= 1024" name="skills-tab">
            </slot>
            <slot v-if="activeTab[tabs[$currentSiteLanguage][0]] || screenWidth >= 1024" name="work-exp-tab">
            </slot>
            <slot v-if="activeTab[tabs[$currentSiteLanguage][2]] || screenWidth >= 1024" name="education-tab">
            </slot>
        </div>
    </div>
</template>